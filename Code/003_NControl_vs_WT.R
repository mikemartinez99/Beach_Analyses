# The purpose of this script is to compare Control Normal pirc rats to WT rats
...
groups <- meta[meta$Group == "Control" | meta$Group == "WT",]$SampleID
...
# Save the dds object
saveRDS(dds, file = "Outputs/003_NControl_vs_WT_Outputs/Rds_Files/Normal_Control_vs_WT_dds.Rds")
...
PCA$Group <- factor(PCA$Group, levels = c("Control", "WT"))
...
plot <- ggplot(PCA, aes(PC1, PC2, fill = Group, color = Group)) +
  ...
  coord_fixed() +
  theme_bw() +
  labs(title = "Normal Control vs WT") +
...
ggsave("Outputs/003_NControl_vs_WT_Outputs/DESeq2/Normal_Control_vs_WT_PCA.tiff", plot, width = 12, height = 8, dpi = 100)
...
write.csv(results, file = "Outputs/003_NControl_vs_WT_Outputs/DESeq2/Normal_Control_vs_WT_DESeq2_Results.csv")
...
GSEA_input$Symbols <- mapIds(org.Rn.eg.db, key = GSEA_input$Ensembl,
                             ...
                             multiVals = "first")
write.csv(GSEA_input, file = "Outputs/003_NControl_vs_WT_Outputs/GSEA/Normal_Control_vs_WT_GESA_Input_List.csv")
...
saveRDS(GO, file = "Outputs/003_NControl_vs_WT_Outputs/Rds_Files/Normal_Control_vs_WT_gseGO.Rds")

# Save results as a dataframe and write to csv
GO.df <- as.data.frame(setReadable(GO, "org.Rn.eg.db", "ENTREZID"))
write.csv(GO.df, file = "Outputs/003_NControl_vs_WT_Outputs/GSEA/Normal_Control_vs_WT_gseGO_results.csv")
...
saveRDS(KEGG, file = "Outputs/003_NControl_vs_WT_Outputs/Rds_Files/Normal_Control_vs_WT_gseKEGG.Rds")

# Save results as a dataframe and write to csv
KEGG.df <- as.data.frame(setReadable(KEGG, "org.Rn.eg.db", "ENTREZID"))
write.csv(KEGG.df, file = "Outputs/003_NControl_vs_WT_Outputs/GSEA/Normal_Control_vs_WT_gseKEGG_results.csv")
