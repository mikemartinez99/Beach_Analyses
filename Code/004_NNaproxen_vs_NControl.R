# The purpose of this script is to analyze normal Pirc Naproxen treated relative to normal control pirc
...
opPath <- "Outputs/004_NNaproxen_vs_NControl_Outputs"
if (!dir.exists(opPath)) {
  dir.create(opPath)
}

# Set up output directory architecture for Rds files
rdsPath <- "Outputs/004_NNaproxen_vs_NControl_Outputs/Rds_Files"
if (!dir.exists(rdsPath)) {
  dir.create(rdsPath)
}

# Set up output diretory architecture for DESeq2 files
ddsPath <- "Outputs/004_NNaproxen_vs_NControl_Outputs/DESeq2"
if (!dir.exists(ddsPath)) {
  dir.create(ddsPath)
}

# Set up output diretory architecture for GSEA files
gseaPath <- "Outputs/004_NNaproxen_vs_NControl_Outputs/GSEA"
...
groups <- meta[meta$Group == "Naproxen" | meta$Group == "Control",]$SampleID
...
# Save the dds object
saveRDS(dds, file = "Outputs/004_NNaproxen_vs_NControl_Outputs/Rds_Files/NNaproxen_vs_NControl_dds.Rds")
...
PCA$Group <- factor(PCA$Group, levels = c("Control", "Naproxen"))
...
plot <- ggplot(PCA, aes(PC1, PC2, fill = Group, color = Group)) +
  ...
  coord_fixed() +
  theme_bw() +
  labs(title = "N Naproxen vs N Control") +
...
ggsave("Outputs/004_NNaproxen_vs_NControl_Outputs/DESeq2/NNaproxen_vs_NControl_PCA.tiff", plot, width = 12, height = 8, dpi = 100)
...
write.csv(results, file = "Outputs/004_NNaproxen_vs_NControl_Outputs/DESeq2/NNaproxen_vs_NControl_DESeq2_Results.csv")
...
GSEA_input$Symbols <- mapIds(org.Rn.eg.db, key = GSEA_input$Ensembl,
                             ...
                             multiVals = "first")
write.csv(GSEA_input, file = "Outputs/004_NNaproxen_vs_NControl_Outputs/GSEA/NNaproxen_vs_NControl_GESA_Input_List.csv")
...
saveRDS(GO, file = "Outputs/004_NNaproxen_vs_NControl_Outputs/Rds_Files/NNaproxen_vs_NControl_WT_gseGO.Rds")

# Save results as a dataframe and write to csv
GO.df <- as.data.frame(setReadable(GO, "org.Rn.eg.db", "ENTREZID"))
write.csv(GO.df, file = "Outputs/004_NNaproxen_vs_NControl_Outputs/GSEA/NNaproxen_vs_NControl_gseGO_results.csv")
...
saveRDS(KEGG, file = "Outputs/004_NNaproxen_vs_NControl_Outputs/Rds_Files/NNaproxen_vs_NControl_gseKEGG.Rds")

# Save results as a dataframe and write to csv
KEGG.df <- as.data.frame(setReadable(KEGG, "org.Rn.eg.db", "ENTREZID"))
write.csv(KEGG.df, file = "Outputs/004_NNaproxen_vs_NControl_Outputs/GSEA/NNaproxen_vs_NControlgseKEGG_results.csv")
